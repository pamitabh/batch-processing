...
save_directory = pathlib.Path(save_directory).joinpath(f"{top_dir.parent.name}_delta_t")
save_directory.mkdir(parents=True, exist_ok=True)
delta_t_per_fish_list = []
delta_t_df_concat = pd.DataFrame()
        ...
        for pos in natsorted([dir for dir in fish_folder.iterdir() if dir.is_dir()]):
            ...
            for imaging in natsorted([dir for dir in pos_folder.iterdir() if dir.is_dir()]):
                ...
                for channel in natsorted([dir for dir in imaging_folder.iterdir() if dir.is_dir()]):
                    ...
                    times[f"{acq_folder_path} {fish} {channel.name} {imaging.name}"] = deepcopy(channel_time)
        
        # all_acq_times.append(times)
        delta_t_df = format_time_df(times)
        delta_t_per_fish_list.append(delta_t_df)
        
    delta_t_df_concat = pd.concat([delta_t_df_concat, delta_t_df], axis=0, ignore_index=True)
delta_t_df_concat.to_csv(save_directory.joinpath(f"delta_t.csv"))