...
# %%
n = (
    input("Enter downscaling factor for x and y dimensions (hit enter for default=4):")
    or 4
)
...
def read_n_downscale_image(read_path):
    ...
    if len(img.shape) == 2:  # 2 dimensional image, e.g. BF image
        img_downscaled = skimage.transform.downscale_local_mean(
            img, (n, n)
...
for root, subfolders, filenames in os.walk(new_src_path):
    for filename in filenames:
        ...
        if (
            ext == "tif" or ext == "tiff"
        ):  # only compress tiff files (prevents compression of other filetypes)
            # print(f'Reading Image: {filepath}')
            fish_num = og_name[og_name.find("fish") + 4]
            save_path = os.path.join(
                new_trg_path, "fish" + str(fish_num)
            )  # save the ds images sorted by fish number
