...
top_dir = input(
    "Enter the path of top dir with all Acquisition folders for original images: "
)
...
save_directory = (
    input("Enter the Save folder for original images [default-Documents]: ")
    or save_directory_default
)

save_directory = pathlib.Path(save_directory).joinpath(
    f"{pathlib.Path(top_dir).name}_delta_t"
)
...
for acq_folder_path in acq_folder_path_list:
    ...
    for fish in natsorted([dir for dir in acq_folder.iterdir() if dir.is_dir()]):
        ...
        for pos in natsorted([dir for dir in fish_folder.iterdir() if dir.is_dir()]):
            ...
            for imaging in natsorted(
                [dir for dir in pos_folder.iterdir() if dir.is_dir()]
            ):
                imaging_folder = pos_folder.joinpath(imaging)
                for channel in natsorted(
                    [dir for dir in imaging_folder.iterdir() if dir.is_dir()]
                ):
                    channel_folder = imaging_folder.joinpath(channel)
                    # channel_time = {}
                    for tp in natsorted(
                        [dir for dir in channel_folder.iterdir() if dir.is_dir()]
                    ):
                        ...
                        try:
                            ...
                            try:
                                time_received = image_meta.image_metadata["UserData"][
                                    "TimeReceivedByCore"
                                ]["scalar"]
                            except KeyError:
                                try:
        ...
        df_one_fish = pd.DataFrame(
            {
                "acquisition_start_time": tp_list,
...
all_df["acquisition_start_time"] = pd.to_datetime(
    all_df["acquisition_start_time"]
)  # str to datetime objects

save_file_path = save_directory.joinpath("acquisition_times.csv")
all_df.to_csv(save_file_path)
print("Successfully saved:", save_file_path)
