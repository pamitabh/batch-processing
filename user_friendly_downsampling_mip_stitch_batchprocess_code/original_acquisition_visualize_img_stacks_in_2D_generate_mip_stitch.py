...
while user_check is None:
    ...
    if user_check.casefold() == "y":
        break
    
    elif user_check.casefold() == "n":
        print("Okay, bye!")
        exit()
...
while user_ch_list is None:
    user_input = input("""Enter the channel numbers to process: 
                        0: Brightfield, 1: GFP, 2: RFP (e.g. '012' for all 3 channels)""") or "0"
...
fish_list = [fish for fish in os.listdir(os.path.join(src, acq_list[0])) if os.path.isdir(os.path.join(src, acq_list[0], fish))]
fish_list = natsorted(fish_list)
pos_list = [pos for pos in os.listdir(os.path.join(src, acq_list[0], fish_list[0])) if os.path.isdir(os.path.join(src, acq_list[0], fish_list[0], pos))]
pos_list = natsorted(pos_list)
timepoint_list = [timepoint for timepoint in os.listdir(os.path.join(src, acq_list[0], fish_list[0], pos_list[0], ch_list[0])) if os.path.isdir(os.path.join(src, acq_list[0], fish_list[0], pos_list[0], ch_list[0], timepoint))]
...
for acq in acq_list:
    for fish in fish_list:
        for ch in ch_list:
            ...
            if 'zstack' not in ch:
                print(f"Skipping {acq}/{fish}/{ch} as it is not a z-stack image")
                continue
            for pos in pos_list:
                for timepoint in timepoint_list:
                        ...
                        for filename in os.listdir(img_folder):
                            ...
                            if (ext == "tif" or ext == "tiff") and (not bpf.check_overflowed_stack(og_name)):
                                ...
                                ski.io.imsave(os.path.join(trg_folder, save_name), img_mip)
    ...
    for fish in fish_list:
        for ch in ch_list:
            ...
            for pos in pos_list:
                for timepoint in timepoint_list:
                        img_folder = os.path.join(src, acq, fish, pos, ch, timepoint)
                        print(f"copying bf for: {img_folder}")
                        trg_folder = os.path.join(new_trg, acq, fish, ch)
                        if not os.path.exists(trg_folder):
                            os.makedirs(trg_folder)
...
for acq in acq_list:
    print(f"Processing {os.path.join(new_trg, acq)}...")
    ch_2Dimg_flags, ch_2Dimg_paths, ch_2Dimg_lists = bpf.find_2D_images(os.path.join(new_trg, acq))
